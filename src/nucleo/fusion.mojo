...
def chain_react(mut state: GameState, center_idx: Int, depth: Int) -> Tuple[Int, Int]:
    if len(state.pieces) < 3:
        return (0, center_idx)
    ...
    var new_center = center_value + 1

    if outer_value >= center_value:
        new_center = outer_value + 2
...
def resolve_plus(mut state: GameState, plus_idx: Int) -> Tuple[Int, Int]:
...
def resolve_black_plus(mut state: GameState, bp_idx: Int) -> Tuple[Int, Int]:
    ...
    var left_value = effective_value(state.pieces[left_idx])
    var right_value = effective_value(state.pieces[right_idx])
    var score = black_plus_score(state.pieces[left_idx], state.pieces[right_idx])
    var merged_value = left_value + 3

    if right_value > left_value:
        merged_value = right_value + 3
...
def resolve_board(mut state: GameState, placement_idx: Int) -> Int:
